...
import {
  ...
  Banknote, Clock, AlertTriangle, Hand, Inbox,
} from "lucide-react";
...
import {
  subscribeOrdersByMotoboy, subscribeAvailableDeliveries,
  updateOrderStatus, claimOrderDelivery,
...
export default function MotoboyPage() {
  ...
  // Entregas atribuídas a este motoboy (suas).
  useEffect(() => {
    if (!allowed) return;
  ...
  // Pool de entregas disponíveis (sem motoboy), que qualquer um pode pegar.
  useEffect(() => {
    if (!allowed) return;
    const unsub = subscribeAvailableDeliveries(
      (list) => setAvailable(list),
      () => toast.error("Não foi possível carregar as entregas disponíveis."),
    );
    return () => unsub();
  }, [allowed]);
  ...
  async function claim(order: Order) {
    ...
    try {
      await claimOrderDelivery(order.id, { uid: user!.uid, name: user!.displayName ?? "Motoboy" });
      toast.success("Entrega aceita! Ela está em \"Minhas\". 🛵");
      setTab("active");
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Não foi possível pegar a entrega.");
    } finally {
      setClaimingId(null);
    }
  }
  ...
  function OrderCard({ order, index, footer }: { order: Order; index: number; footer?: ReactNode }) {
  ...
  const tabs: { key: typeof tab; label: string; count: number }[] = [
    { key: "available", label: "Disponíveis", count: pool.length },
    { key: "active", label: "Minhas", count: active.length },
    ...
    <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-2xl font-black text-[var(--color-text-primary)]">Entregas</h1>
            <p className="text-sm text-[var(--color-text-muted)]">
              Olá, {user.displayName?.split(" ")[0]} · {pool.length} disponíve{pool.length !== 1 ? "is" : "l"} · {active.length} sua{active.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 text-xs font-medium text-[var(--color-success)] mb-5 mt-3">
          ...
          Ao vivo · novas entregas aparecem aqui automaticamente
        </div>
        ...
        <div className="flex gap-2 mb-6">
          {tabs.map(t => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === t.key ? "bg-[var(--color-neon-blue)] text-white" : "bg-[var(--color-bg-overlay)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"}`}
            >
              {t.label} ({t.count})
            </button>
          ))}
          ...
          pool.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-[var(--color-text-muted)]">Nenhuma entrega disponível no momento.</p>
              </CardContent>
            </Card>
          ) : (
                    ...
                    <div className="pt-1">
                      ...
                      >
                        {claimingId === order.id
                          ...
                          : <><Hand className="w-4 h-4" /> Pegar entrega</>}
                      </Button>
                    </div>
                  }
                />
                ...
                <Card>
                  <CardContent className="flex flex-col items-center justify-center py-16 gap-3">
                    ...
                    <p className="text-[var(--color-text-muted)]">
                      {tab === "active" ? "Você ainda não tem entregas. Pegue uma na aba \"Disponíveis\"." : "Nenhuma entrega concluída ainda."}
                    </p>
                  </CardContent>
                </Card>
              );
